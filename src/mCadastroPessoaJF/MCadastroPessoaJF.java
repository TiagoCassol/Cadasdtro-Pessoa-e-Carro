...
import controller.CVagaEstacionamento;
...
public class MCadastroPessoaJF {
    ...
    public static void atualizaCarro() {
        System.out.println("-- Atualiza Carro --");
        System.out.println("");
        ...
        if (cadCarros.verPlaca(placa)) {
            Carro c = new Carro();
            Pessoa p = new Pessoa();
            c = cadCarros.getByDoc(placa);
            System.out.println("Atualizando Carro: "
                    + "\nProprietário: " + cadPessoas.getNomePes(c.getIdPessoa())
                    + "\nPlaca: " + c.getPlaca()
                    + "\nMarca: " + c.getMarca()
                    + "\nModelo: " + c.getModelo()
                    + "\nCor: " + c.getCor()
            ...
            do {
                ...
                System.out.println("3 - Atualizar Propritário");
                System.out.println("Escolha um opção:");
                int op = ler.nextInt();
                switch (op) {
                    case 1:
                        System.out.print("Informe a cor: ");
                        c.setCor(leTexto());
                        System.out.print("\nInforme o proprietario: ");
                        boolean cpfPess;
                        do {
                            System.out.print("\nInforme o cpf do novo proprietario: ");
                            String cpf = leTexto();
                            cpfPess = cadPessoas.verCPF(cpf);
                            if (cpfPess) {
                                c.setIdPessoa(cadPessoas.pesqIdPes(cpf));
                            } else {
                                System.out.println("CPF inválido, tente novamente!");
            ...
            System.out.println("Deseja cadastrar esta placa? 1-Sim | 0-Não");
            int c = ler.nextInt();
            if (c == 1) {
                cadCarro();
            }
    ...
    public static void deletarCarro() {
        ...
        do {
            ...
            if (cadCarros.verPlaca(placa)) {
                Carro c;
                c = cadCarros.getByDoc(placa);
                System.out.println("Deseja realmente deletar este carro? "
                        + "\nProprietário: " + cadPessoas.getNomePes(c.getIdPessoa())
                        + "\nPlaca: " + c.getPlaca()
                        + "\nMarca: " + c.getMarca()
                        + "\nModelo: " + c.getModelo()
